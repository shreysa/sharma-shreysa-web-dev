module.exports = function(app, models) {
  ...
  app.get(
    "/api/project/restaurant/review/like/follow/:restaurantId",
    findRestaurant
  );
  ...
  app.get(
    "/api/projectuser/user/:userId/restaurant/review/:restaurantId",
    getReviewByUserId
  );
  app.get("/api/projectuser/user/:userId", findAllReviewsByUserId);
  app.get(
    "/api/projectuser/:restaurantId/reviews",
    findAllReviewsByRestaurantId
  );
  app.delete("/api/projectuser/:reviewId", deleteReview);
  app.put(
    "/api/projectuser/review/:reviewId/restaurant/:reviewText",
    updateReview
  );
  app.get("/api/projectuser/review/restaurant", getAllReviews);

  function findRestaurant(req, res) {
    restaurantModel.findRestaurantByRestaurantId(req.params.restaurantId).then(
      function(restObj) {
        res.json(restObj);
      },
      function(error) {
  ...
  function deleteReviewByUserId(req, res) {
    reviewModel.deleteReviewByUserId(req.params.userId).then(
      function(stats) {
        console.log(stats);
        res.send(200);
      },
  ...
  function getAllReviews(req, res) {
    reviewModel.getAllReviews().then(
      function(reviewObj) {
        res.json(reviewObj);
      },
      function(error) {
  ...
  function updateReview(req, res) {
    reviewModel.updateReview(req.params.reviewId, req.params.reviewText).then(
      function(stats) {
        console.log(stats);
        res.send(200);
      },
  ...
  function deleteReview(req, res) {
    reviewModel.deleteReview(req.params.reviewId).then(
      function(stats) {
        console.log(stats);
        res.send(200);
      },
  ...
  function addReview(req, res) {
    ...
    restaurantModel.findRestaurant(restaurantId).then(
        ...
        if (restObj == null) {
          restaurantModel.addRestaurant(restForDb).then(
            function(addRestObj) {
              ...
              reviewModel.addReview(review).then(
                function(reviewObj) {
                  res.json(reviewObj);
                },
                function(err) {
          ...
          categoryModel.findRestaurant(restDbId).then(function(restObj, error) {
            if (restObj == null) {
              categoryModel.addCategoryRestaurant(catData).then(
                function(addCatObj) {
                  res.json(addCatObj);
  ...
  function findAllReviewsByRestaurantId(req, res) {
    reviewModel.findAllReviewsByRestaurantId(req.params.restaurantId).then(
  ...
  function findAllReviewsByUserId(req, res) {
    reviewModel.findAllReviewsByUserId(req.params.userId).then(
      function(restReviewObj) {
        res.json(restReviewObj);
      },
      function(error) {
